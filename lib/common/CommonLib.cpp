...
snprintf(char *buf, size_t bufSize, const char *fmt, ...)
{
    va_list va;
    va_start(va, fmt);
    size_t len = _vprintf(fmt, 0, buf, 10, va, bufSize ? bufSize - 1 : 0);
    if (buf && bufSize) {
        buf[len] = 0;
    }
    return len;
 ...
 * order.
 *
 * @return An optional length and a pointer to the last character
 * written in the buffer (i.e., the first character of the string).
 * The buffer pointed to by @a nbuf must have length >= MAXNBUF.
 */
static char *
_sprintn(char *nbuf, u64 num, int base, int *lenp, bool upper)
{
    char *p, c;
 ...
 *
 * Print formatted string through the specified callback function @a func.
 *
 * @param fmt Format string.
 * @param func Callback function to call for each output character.
 * @param arg Argument for callback function @a func.
 * @param radix Radix for numbers representation printed by format specifier @%r.
 * @param ap Arguments list for format string.
 * @param maxOut Maximal number of characters to output.
 *
 * @return Number of characters printed.
 */
size_t
_vprintf(const char *fmt, PutcFunc func, void *arg, int radix, va_list ap,
         int maxOut)
{
    /* Max number conversion buffer length: a u_quad_t in base 2, plus null byte. */
    const int MAXNBUF = sizeof(i64) * NBBY + 1;
    char nbuf[MAXNBUF];
    char *d;
    ...
    u64 num;
    int base, tmp, width;
    bool lflag, qflag, ladjust, sharpflag, neg, sign, dot, upper, cflag, hflag,
         jflag, tflag, zflag, stop =  false;
    int dwidth;
    char padc;
    int retval = 0;
