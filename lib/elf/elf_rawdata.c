...
elf_rawdata(Elf_Scn *scn, Elf_Data *data) {
    ...
    else if (!(sd = (Scn_Data*)elf_malloc(sizeof(*sd)))) {
    ...
    *sd = _elf_data_init;
    ...
    if (scn->s_type != SHT_NOBITS && scn->s_size) {
        if (!(sd->sd_memdata = (char*)elf_malloc(scn->s_size))) {
        seterr(ERROR_IO_2BIG);
        elf_mfree(sd);
        return NULL;
        }
        else if (elf->e_rawdata) {
        elf_memcpy(sd->sd_memdata, elf->e_rawdata + scn->s_offset, scn->s_size);
        }
        else if (!_elf_read(elf, sd->sd_memdata, scn->s_offset, scn->s_size)) {
        elf_mfree(sd->sd_memdata);
        elf_mfree(sd);
        return NULL;
        }
