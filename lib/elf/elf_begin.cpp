...
Elf_Malloc      elf_malloc;
Elf_Mrealloc    elf_mrealloc;
Elf_Mfree       elf_mfree;
...
Elf*
elf_begin(Elf_File *fd, Elf_Cmd cmd, Elf *ref)
{
    ...
    if (_elf_version == EV_NONE) {
        ...
        size = arhdr->ar_size;
    } else if ((off = fd->Seek(fd, 0, ELF_SEEK_END)) == (off_t) -1
        || (off_t) (size = off) != off) {
        seterr(ERROR_IO_GETSIZE);
        return NULL;
    }
    if (!(elf = (Elf*)elf_malloc(sizeof(Elf)))) {
