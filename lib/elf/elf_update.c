...
static int _elf_update_pointers(Elf *elf, char *outbuf, size_t len)
{
    ...
    if (len <= elf->e_dsize) {
        ...
        data = elf->e_data;
    } else if ((data = (char*)elf_mrealloc(elf->e_data, len))) {
        elf->e_dsize = len;
    } else {
    ...
    if (elf->e_rawdata) {
        /* update raw image */
        if (!(rawdata = (char*)elf_mrealloc(elf->e_rawdata, len))) {
            seterr(ERROR_IO_2BIG);
            return -1;
        }
    ...
    for (scn = elf->e_scn_1; scn; scn = scn->s_link) {
        ...
        if ((sd = scn->s_data_1)) {
            ...
            if (sd->sd_memdata && !sd->sd_free_data) {
                elf_assert(
                    ptrinside(sd->sd_memdata, elf->e_data, elf->e_dsize));
        ...
        if ((sd = scn->s_rawdata)) {
            ...
            if (sd->sd_memdata && sd->sd_free_data) {
                ...
                if (!(rawdata = (char*)elf_mrealloc(sd->sd_memdata, len))) {
                    seterr(ERROR_IO_2BIG);
                    return -1;
                }
...
static int xwrite(Elf_File *fd, char *buffer, size_t len)
{
    size_t done = 0;
    size_t n;

    while (done < len) {
        n = fd->Write(fd, buffer + done, len - done);
        if (n == 0) {
            /* file system full */
            return -1;
        } else {
...
static off_t _elf_output(Elf *elf, Elf_File *fd, size_t len,
