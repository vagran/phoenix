...
static int
xread(Elf_File *fd, char *buffer, size_t len) {
    size_t done = 0;
    size_t n;

    while (done < len) {
        n = fd->Read(fd, buffer + done, len - done);
        if (n == 0) {
...
_elf_read(Elf *elf, void *buffer, size_t off, size_t len) {
    ...
    if (elf->e_disabled) {
        ...
        off += elf->e_base;
        if (elf->e_fd->Seek(elf->e_fd, (off_t)off, ELF_SEEK_SET) != (off_t)off) {
            seterr(ERROR_IO_SEEK);
        } else if (!(tmp = buffer) && !(tmp = elf_malloc(len))) {
